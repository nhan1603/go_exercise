	...
	for s, tc := range tcs {
		t.Run(s, func(t *testing.T) {
			testutil.WithTxDB(t, func(dbConn pg.BeginnerExecutor) {
				...
				id, errr := userRepo.CreateUser(context.Background(), tc.sendEmail)

				id1, err1 := userRepo.CreateUser(context.Background(), tc.subscribeEmail)

				id2, err2 := userRepo.CreateUser(context.Background(), tc.friendEmail)

				id3, err3 := userRepo.CreateUser(context.Background(), tc.blockEmail)

				_, err4 := userRepo.CreateUser(context.Background(), tc.mentionedEmail[0])
