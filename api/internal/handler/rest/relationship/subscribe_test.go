...
func TestApiHandler_Subscribe(t *testing.T) {
	tcs := map[string]struct {
		...
		"failFromDB": {
			...
			expErr:    &httpserv.Error{Status: http.StatusInternalServerError, Code: "internal_error", Desc: "cannot subscribe user"},
	...
	for name, tc := range tcs {
		t.Run(name, func(t *testing.T) {
			r := httptest.NewRequest(http.MethodPost, "/api/subscribe", bytes.NewReader(tc.body))
